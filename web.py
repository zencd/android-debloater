...
APP_START_DIR = Path.cwd()
# APKS_DIR = APP_START_FOLDER / 'apks_pv'
...
def serve_read_device_apps_meta(request, response):
    APP_META_FILE.load()
    APP_META_FILE.data = APP_META_FILE.data if APP_META_FILE.data else dict()
    app_meta_packages = APP_META_FILE.data.get('packages')
    if not app_meta_packages:
        app_meta_packages = dict()
        APP_META_FILE.data['packages'] = app_meta_packages
    ...
    try:
        for package in list_device_all_packages():
            ...
            if not apk_path:
                print(f'WARN: cannot find main apk for {package}')
                fails += 1
                continue
            ...
            os.chdir(tmp_app_folder)
            cmd = ['adb', 'pull', apk_path]
            # todo if failed then resolve it as stem
            rc, stdout, stderr = exec_(cmd, stdout=None, stderr=None)
            if rc != 0:
            ...
            if not app_title or app_title == 'base':
                ...
                fails += 1
                continue
            # icon = apk_info.get_app_icon()
            app_icon_file_short = ''
            try:
                icon_data = apk_info.icon_data
                icon_info = apk_info.icon_info
            ...
            print('  version_name:', apk_info.version_name)
            pdata = {'title': app_title,
                     'version_name': apk_info.version_name,
                     'icon': app_icon_file_short}
            if app_icon_file_short:
                pdata['icon'] = app_icon_file_short
            app_meta_packages[package] = pdata
            APP_META_FILE.dump()
            oks += 1
        response.content_type = CT_JSON
